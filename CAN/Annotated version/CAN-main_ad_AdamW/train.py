...
parser = argparse.ArgumentParser(description='model training')  # 创建一个 ArgumentParser 对象，用于接收命令行参数。
# description 参数： 命令行帮助信息中显示的描述文本
parser.add_argument('--dataset', default='CROHME', type=str, help='数据集名称')
parser.add_argument('--check', action='store_true', help='测试代码选项')
args = parser.parse_args()  # 解析命令行输入的参数，并将它们存储在 args 对象中
...
model = CAN(params) # 定义使用的模型
now = time.strftime("%Y-%m-%d-%H-%M", time.localtime())
model.name = f'{params["experiment"]}_{now}_decoder-{params["decoder"]["net"]}'  # 定义模型名，用于存储检查点
...
if args.check:
    ...
    writer = SummaryWriter(f'{params["log_dir"]}/{model.name}') # SummaryWriter 是 TensorBoard 的接口，用于将训练过程中的各种数据（如损失、准确率）记录并可视化。
...
if params['finetune']: # 'finetune'为True，表示当前训练不是从头开始，而是基于一个预训练模型进行微调（fine-tuning）
    print('加载预训练模型权重')
...
if args.dataset == 'CROHME':
    ...
    for epoch in range(init_epoch, params['epochs']):
        ...
        # 评价指标：损失loss，单词准确率word_score，表达式准确率exprate
